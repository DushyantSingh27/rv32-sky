...
| **M3** | **RV32I core integration** | Leaf blocks assembled into the 5-stage pipeline. **RV32I only** - C extension and traps are M4. Compliance suite passing in Verilator (RISCOF vs ACT4 to be determined - see ADR-0003). **Sail** lockstep working (not Spike). riscv-formal integrated and passing. CI running L0+L1 on every commit. See ADR-0003 for the staged plan. |
