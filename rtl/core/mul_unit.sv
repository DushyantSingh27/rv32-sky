...
(
  ...
  always_comb begin
    ...
    a_is_signed = (op == MD_MUL) || (op == MD_MULH) || (op == MD_MULHSU);
    b_is_signed = (op == MD_MUL) || (op == MD_MULH);
    neg_a       = a_is_signed && a[XLEN-1];
    neg_b       = b_is_signed && b[XLEN-1];
    a_mag       = neg_a ? (~a + 32'd1) : a;
    b_mag       = neg_b ? (~b + 32'd1) : b;
  end
